...
var hostId = crypto.createHash('md5').update(hostname + __dirname).digest('hex')
...
function processVerification (reporter, definition, m) {
  ...
  return fs.readFileAsync(licenseInfoPath, 'utf8').catch(function () {
    ...
    if (hash !== crypto.createHash('md5').update(JSON.stringify(storedLicense)).digest('hex')) {
...
function verifyInService (reporter, definition, m, licenseInfoPath) {
  ...
  return isLicenseKeyStoredInNegativeCache(reporter, m.licenseKey).then(function (cachedNegativeResponse) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      request({
        url: 'http://local.net/license-key',
        method: 'POST',
        body: m,
        json: true
        ...
        // the remote validator rejected the license key, the jsreport instance should failt to start
        if (body.status === 1) {
        ...
        if (body.type !== 'trial' && body.type !== 'free' && definition.options.useSavedLicenseInfo !== false) {
          ...
          licensingInfo.securityHash = crypto.createHash('md5').update(JSON.stringify(licensingInfo)).digest('hex')
...
function isLicenseKeyStoredInNegativeCache (reporter, licenseKey) {
  var licenseCacheFile = path.join(reporter.options.tempDirectory, 'licensing', licenseKey + '.txt')

  return mkdirpAsync(path.join(reporter.options.tempDirectory, 'licensing')).then(function () {
    return statAsync(licenseCacheFile).then(function (s) {
      if ((new Date(s.mtime).getTime() + 10000) < new Date().getTime()) {
        return null
      }
      return readFileAsync(licenseCacheFile, 'utf8')
    })
  }).catch(function () {
    // the cache file doesn't neet to exist, or have wrong permissions...
...
function writeLicenseKeyToNegativeCache (reporter, licenseKey, message) {
  return writeFileAsync(path.join(reporter.options.tempDirectory, 'licensing', licenseKey + '.txt'), message).catch(function () {
  })
}
