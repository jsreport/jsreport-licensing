...
async function verifyInService (reporter, definition, m, licenseInfoPath) {
  ...
  async function handleResponse (err, response) {
    if (isDone) {
      return
    }
    ...
    if (err || response == null || response.status !== 200 || response.data == null || !Number.isInteger(response.data.status)) {
  ...
  try {
    ...
    if (isDone) {
      // the error comes from the handleResponse
      throw err
    }

    if (err.response) {
      return handleResponse(null, err.response)
    }
